...
function ServiceCard({ icon, title, price, description, artists, linkTo, linkLabel }) {
  return (
    <div
    ...
    >
      ...
      <Link to={linkTo} className="text-sm text-metallic-gold hover:text-firebrick transition-colors mt-auto">
        {linkLabel}
      </Link>
    </div>
  );
    ...
    <div className="min-h-screen bg-black text-gray-100">
      ...
      <section className="py-20 bg-black">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-12 text-metallic-gold">{t.home.testimonialsTitle}</h2>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
            <div className="flex flex-col items-center">
              ...
              <h3 className="text-xl font-bold mb-2">{t.home.trustLicensed}</h3>
              <p className="text-gray-300">{t.home.trustLicensedSub}</p>
            </div>
            <div className="flex flex-col items-center">
              <Award className="w-12 h-12 text-metallic-gold mb-4" />
              <h3 className="text-xl font-bold mb-2">{t.home.trustAward}</h3>
              <p className="text-gray-300">{t.home.trustAwardSub}</p>
            </div>
            <div className="flex flex-col items-center">
              <Users className="w-12 h-12 text-metallic-gold mb-4" />
              <h3 className="text-xl font-bold mb-2">{t.home.trustClients}</h3>
              <p className="text-gray-300">{t.home.trustClientsSub}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 bg-black">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-12 text-metallic-gold">{t.home.stylesTitle}</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5 justify-items-center mx-auto max-w-7xl">
            <ServiceCard
              ...
              title={t.home.norseTitle}
              price={t.home.norsePrice}
              description={t.home.norseDesc}
              artists={["Imre"]}
              linkTo="/portfolio"
              linkLabel={t.home.portfolioLink}
            />
            <ServiceCard
              icon={<Pen className="w-8 h-8 text-firebrick" />}
              title={t.home.fineLineTitle}
              price={t.home.fineLinePrice}
            ...
            />
            <ServiceCard
              icon={<ImageIcon className="w-8 h-8 text-firebrick" />}
              title={t.home.realismTitle}
              price={t.home.realismPrice}
              description={t.home.realismDesc}
              artists={["Imre"]}
              linkTo="/portfolio"
              linkLabel={t.home.portfolioLink}
            />
            <ServiceCard
              icon={<Palette className="w-8 h-8 text-firebrick" />}
              title={t.home.customTitle}
              price={t.home.customPrice}
              description={t.home.customDesc}
              artists={["Imre", "Eszter"]}
              linkTo="/portfolio"
              linkLabel={t.home.portfolioLink}
            />
            <ServiceCard
              icon={<Brush className="w-8 h-8 text-firebrick" />}
              title={t.home.airbrushTitle}
              price={t.home.airbrushPrice}
              description={t.home.airbrushDesc}
              artists={["Imre"]}
              linkTo="/booking#form"
              linkLabel={t.home.inquireLink}
            />
          </div>
          ...
          <div className="mt-12 bg-black/40 backdrop-blur-sm border border-metallic-gold/20 text-white py-3 px-6 rounded-lg max-w-2xl mx-auto">
            <p className="text-center text-lg font-semibold italic">
              <span className="text-metallic-gold">{t.home.servicesDisclaimer}</span>
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 bg-black" id="form">
        <div className="container mx-auto px-4">
          <div className="max-w-2xl mx-auto bg-viking-navy p-8 rounded-lg shadow-[0_0_20px_rgba(30,58,95,0.5)]">
            <h2 className="text-3xl font-bold text-center mb-8 text-metallic-gold">{t.home.formTitle}</h2>
            <ContactForm />
          </div>
        </div>
      </section>
